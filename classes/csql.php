	...
	*/
	...
	protected function getOdb($engine, $file) {
		...
		switch($engine) {
			...
			break;
		    case "php":
				 // $odb = new PDO('mysql=host:localhost;dbname:hide_bd','root','pantera');
...
//	echo "</PRE>";
		...
		$where = isset($where) ? "where $where" : "";
                $searchall = isset($buscar) ? $buscar : $search;
		if($query && is_array($searchall) && sizeof($searchall)) {
			$a = array();
			foreach($searchall as $f) {
				$a[] = "$f regexp '$query'";
			}
	...
	/**
	  ...
	  */
	public function __construct($engine = "php", $file = "db.sqlite") {
